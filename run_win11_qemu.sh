...
# PulseAudio backend and Intel HDA audio hardware
# Only configure PulseAudio if a PulseAudio server appears available to avoid
# qemu trying to stat a non-existent pid/socket (common in containers).
PULSE_AVAILABLE=0
# prefer pactl to check server, fallback to checking $XDG_RUNTIME_DIR/pulse sockets/files
if command -v pactl >/dev/null 2>&1 && pactl info >/dev/null 2>&1; then
  PULSE_AVAILABLE=1
elif [[ -S "${XDG_RUNTIME_DIR:-}/pulse/native" || -f "${XDG_RUNTIME_DIR:-}/pulse/pid" ]]; then
  PULSE_AVAILABLE=1
fi
if [[ $PULSE_AVAILABLE -eq 1 ]]; then
  QEMU_CMD+=( -audiodev pa,id=pa )
  QEMU_CMD+=( -device intel-hda -device hda-output,audiodev=pa -device hda-input,audiodev=pa )
else
  echo "PulseAudio not available; skipping audio device setup to avoid pidfile errors."
fi
...
# SPICE server: listen on specified address:5900 with no authentication
QEMU_CMD+=( -spice port=5900,addr=${SPICE_ADDR},disable-ticketing=on )

# Do not enable VNC — use SPICE. Keep display hidden (headless) by default.
