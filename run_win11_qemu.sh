...
#                        [--spice-port PORT] [--vnc-port PORT]
# Example:
#   ./run_win11_qemu.sh --name win11-22h2 --mem 8G --disk-size 64G
#   ./run_win11_qemu.sh --spice-port 5900 --vnc-port 5901

ISO_URL_DEFAULT="https://github.com/Rayyan-Sta/wincloud/releases/download/v2.0/Micro11.iso"
...
NO_DOWNLOAD=0
SECOND_DISK=""
ENABLE_SECOND_DISK=1
CREATE_SECOND_DISK=1
...
while [[ "$#" -gt 0 ]]; do
  case "$1" in
    ...
    --second-disk) SECOND_DISK="$2"; shift 2;;
    --no-second-disk) ENABLE_SECOND_DISK=0; shift;;
    --no-create-second-disk) CREATE_SECOND_DISK=0; shift;;
...
# Secondary data disk (AHCI)
# If the user provided --second-disk, use that path; otherwise keep default in the vm workspace
if [[ -z "${SECOND_DISK:-}" ]]; then
  SECOND_DISK="$WORK_DIR/${VM_NAME}_data.qcow2"
fi
if [[ "$ENABLE_SECOND_DISK" -eq 1 ]]; then
  if [[ ! -f "$SECOND_DISK" ]]; then
    if [[ "$CREATE_SECOND_DISK" -eq 1 ]]; then
      echo "Creating secondary disk $SECOND_DISK (2G)..."
      mkdir -p "$(dirname "$SECOND_DISK")" 2>/dev/null || true
      qemu-img create -f qcow2 "$SECOND_DISK" 2G
    else
      echo "Secondary disk '$SECOND_DISK' does not exist and --no-create-second-disk was specified." >&2
      echo "Either create the file manually or remove --no-create-second-disk. Aborting." >&2
      exit 1
    fi
  fi
fi
...
if [[ $USE_KVM -eq 1 ]]; then
  if [[ -c /dev/kvm ]]; then
    if [[ -r /dev/kvm && -w /dev/kvm ]]; then
      ...
      echo "Trying to enable KVM..."
      if sudo chmod 666 /dev/kvm 2>/dev/null; then
        echo "KVM successfully enabled."
        QEMU_CMD+=( -enable-kvm )
      else
        echo "KVM hasn't been successfully enabled. Falling back to TCG."
        USE_KVM=0
      fi
    fi
  else
...
# Boot order: boot once from CD-ROM (d), then fallback to hard disk
QEMU_CMD+=( -boot once=d )

# OVMF pflash configuration
QEMU_CMD+=( -drive if=pflash,format=raw,readonly=on,file="$OVMF_CODE" -drive if=pflash,format=raw,file="$VARS_COPY" )

# Drives and devices
QEMU_CMD+=( -device virtio-scsi-pci,id=scsi0 )
QEMU_CMD+=( -drive file="$DISK_IMG",if=none,id=hd0,format=qcow2 )
QEMU_CMD+=( -device scsi-hd,drive=hd0 )

# Attach secondary AHCI disk via ich9-ahci + ide-hd
if [[ "$ENABLE_SECOND_DISK" -eq 1 ]]; then
  QEMU_CMD+=( -device ich9-ahci,id=ahci0 )
  QEMU_CMD+=( -drive file="$SECOND_DISK",if=none,id=data0,format=qcow2 )
  QEMU_CMD+=( -device ide-hd,drive=data0,bus=ahci0.0 )
fi
...
# VGA, input, audio, and SPICE integration
# Use QXL for SPICE graphics, add USB tablet for pointer
QEMU_CMD+=( -vga qxl -device usb-ehci -device usb-tablet )

# SPICE audio backend - works over SPICE connection for remote audio
# This allows audio to work through SSH tunnels and remote connections
QEMU_CMD+=( -audiodev spice,id=spiceaudio )
QEMU_CMD+=( -device intel-hda -device hda-duplex,audiodev=spiceaudio )

# virtio-serial + virtserialport for SPICE vdagent (clipboard/clipboard support)
QEMU_CMD+=( -device virtio-serial-pci -device virtserialport,chardev=ch1,id=ch1,name=com.redhat.spice.0 -chardev qemu-vdagent,id=ch1,name=vdagent,clipboard=on )

# SPICE server: listen on specified address and port with no authentication
QEMU_CMD+=( -spice port=${SPICE_PORT},addr=${SPICE_ADDR},disable-ticketing=on )

# Enable VNC for additional connectivity
# VNC display number is calculated as: port - 5900
# E.g., port 5901 = display :1, port 5902 = display :2
VNC_DISPLAY=$((VNC_PORT - 5900))
QEMU_CMD+=( -vnc ${SPICE_ADDR}:${VNC_DISPLAY} )

# Ensure XDG_RUNTIME_DIR is set for QEMU (some systems/containers don't set it)
if [[ -z "${XDG_RUNTIME_DIR:-}" ]]; then
  XDG_RUNTIME_DIR="$WORK_DIR/xdg-runtime-$(id -u)"
  mkdir -p "$XDG_RUNTIME_DIR" 2>/dev/null || true
  chmod 700 "$XDG_RUNTIME_DIR" 2>/dev/null || true
  export XDG_RUNTIME_DIR
  echo "Note: XDG_RUNTIME_DIR was not set; using $XDG_RUNTIME_DIR"
fi
...
# Run QEMU, but filter common PipeWire/pw.conf warnings from stderr to reduce noise
# Some container environments have no PipeWire/PulseAudio config and the library
# emits harmless warnings; filter them so users see only relevant errors.
if [[ -n "${BASH_VERSION:-}" ]]; then
  # Use process substitution to filter stderr lines matching pw.conf/client.conf
  "${QEMU_CMD[@]}" 2> >(grep -v -E 'pw.conf|client.conf' >&2)
else
  "${QEMU_CMD[@]}"
fi
